...
archive="$first_dir/boxup-0.2.0.tar.gz"
...
SOURCE_DATE_EPOCH=0 BOXUP_OUTPUT_DIR="$second_dir" \
  sh "$project_dir/scripts/make-source-archive.sh" >/dev/null
cmp -s "$archive" "$second_dir/boxup-0.2.0.tar.gz" || {
  printf '%s\n' 'Source archive is not reproducible.' >&2
  exit 1
}
cmp -s "$archive.sha256" "$second_dir/boxup-0.2.0.tar.gz.sha256"
(cd "$first_dir" && sha256sum -c boxup-0.2.0.tar.gz.sha256)

tar -tzf "$archive" >"$listing"
if grep -Eq '^boxup-0\.2\.0/(target|dist|scripts/__pycache__)/|\.py[co]$|^boxup-0\.2\.0/packaging/arch/(pkg|src)/' "$listing"; then
  printf '%s\n' 'Source archive contains generated build artifacts.' >&2
  exit 1
fi
if grep -Fq "boxup-0.2.0/$secret_relative/" "$listing"; then
...
for forbidden in plan.md .github/workflows/ci.yml packaging/arch/PKGBUILD; do
  if grep -Fqx "boxup-0.2.0/$forbidden" "$listing"; then
    printf 'Source archive contains non-release file: %s\n' "$forbidden" >&2
    exit 1
  fi
done
grep -Fqx 'boxup-0.2.0/Cargo.lock' "$listing"
grep -Fqx 'boxup-0.2.0/.cargo/config.toml' "$listing"
grep -Fqx 'boxup-0.2.0/AGENTS.md' "$listing"
grep -Fqx 'boxup-0.2.0/SECURITY.md' "$listing"
grep -Eq '^boxup-0\.2\.0/vendor/[^/]+/Cargo\.toml$' "$listing"
tar -C "$extract_dir" -xzf "$archive"
mkdir "$work_dir/cargo-home" "$work_dir/target"
CARGO_HOME="$work_dir/cargo-home" CARGO_TARGET_DIR="$work_dir/target" \
  CARGO_NET_OFFLINE=true cargo check --manifest-path "$extract_dir/boxup-0.2.0/Cargo.toml" \
  --frozen --offline
